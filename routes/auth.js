...
router.post('/register', async (req, res) => {
  try {
    ...
    if (existingUser && (!existingUser.password || existingUser.password === '')) {
      ...
      return res.status(200).json({ message: 'Account restored', token, user: { id: existingUser._id, name: existingUser.name, email: existingUser.email, role: existingUser.role } });
    }
    if (existingUser) {
    ...
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(password, salt);
    const user = new User({ name, email: normalizedEmail, password: hashedPassword, role: assignedRole });
    await user.save();
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user.password || user.password === '') {
      return res.status(400).json({ message: 'Account password missing. Please sign up again to restore your account.' });
    }
